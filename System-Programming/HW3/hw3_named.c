...
int main(int argc, char const *argv[])
{
	...
	*chef_check = 0;
	...
	sem_t* chef_semaphores[6];
	for (int i = 0; i < 6; ++i)
	{
		char temp[25];
		sprintf(temp,"%s_%d", argv[4],i);
		chef_semaphores[i] = sem_open(temp,O_CREAT,S_IRUSR | S_IWUSR, 0);
	}
	
	char temp2[25];
	sprintf(temp2,"%s", argv[6]);
	sem_t* wholesaler_semaphore = sem_open(temp2, O_CREAT,S_IRUSR | S_IWUSR, 0);
	sem_t* mutex_semaphore = sem_open("/mutex_semaphore",O_CREAT,S_IRUSR | S_IWUSR, 1);
		...
		(*chef_check)++;
			...
			while(1)
			{
				...
				if(i == 0)
				{
					...
					if((delivered_ingredients[0] == 'M' && delivered_ingredients[1] == 'F') || (delivered_ingredients[0] == 'F' && delivered_ingredients[1] == 'M'))
					{	
				...
				else if(i == 1)
				{
					...
					if((delivered_ingredients[0] == 'M' && delivered_ingredients[1] == 'W') || (delivered_ingredients[0] == 'W' && delivered_ingredients[1] == 'M'))							
					{
						printf("chef%d (pid %d) has taken the Milk (%s)\n", i, getpid(), delivered_ingredients);						
				...
				else if(i == 2)
				{
					...
					if((delivered_ingredients[0] == 'M' && delivered_ingredients[1] == 'S') || (delivered_ingredients[0] == 'S' && delivered_ingredients[1] == 'M'))							
					{
						printf("chef%d (pid %d) has taken the Milk (%s)\n", i, getpid(), delivered_ingredients);						
				...
				else if(i == 3)
				{
					...
					if((delivered_ingredients[0] == 'F' && delivered_ingredients[1] == 'W') || (delivered_ingredients[0] == 'W' && delivered_ingredients[1] == 'F'))							
					{
						printf("chef%d (pid %d) has taken the Flour (%s)\n", i, getpid(), delivered_ingredients);						
						printf("chef%d (pid %d) has taken the Walnut (%s)\n", i, getpid(), delivered_ingredients);	
						printf("chef%d (pid %d) is preparing the dessert (%s)\n", i, getpid(), delivered_ingredients);
				...
				else if(i == 4)
				{
					...
					if((delivered_ingredients[0] == 'F' && delivered_ingredients[1] == 'S') || (delivered_ingredients[0] == 'S' && delivered_ingredients[1] == 'F'))							
					{
						printf("chef%d (pid %d) has taken the Flour (%s)\n", i, getpid(), delivered_ingredients);						
						printf("chef%d (pid %d) has taken the Sugar (%s)\n", i, getpid(), delivered_ingredients);	
						printf("chef%d (pid %d) is preparing the dessert (%s)\n", i, getpid(), delivered_ingredients);
				...
				else if(i == 5)
				{
					...
					if((delivered_ingredients[0] == 'W' && delivered_ingredients[1] == 'S') || (delivered_ingredients[0] == 'S' && delivered_ingredients[1] == 'W'))
					{							
						printf("chef%d (pid %d) has taken the Walnut (%s)\n", i, getpid(), delivered_ingredients);						
						printf("chef%d (pid %d) has taken the Sugar (%s)\n", i, getpid(), delivered_ingredients);
						printf("chef%d (pid %d) is preparing the dessert (%s)\n", i, getpid(), delivered_ingredients);	
	...
	{
		char temp[25];
		sprintf(temp,"%s_%d", argv[8],i);
		pusher_semaphores[i] = sem_open(temp,O_CREAT,S_IRUSR | S_IWUSR, 0);
	}
	...
	sem_close(wholesaler_semaphore);
	sem_close(mutex_semaphore);

	sem_unlink("/mutex_semaphore");
	sem_unlink("/wholesaler_semaphore");

	for (int i = 0; i < 6; ++i)
	{
		char temp[30];
		sprintf(temp,"%s_%d", argv[4],i);
		sem_close(chef_semaphores[i]);
		sem_unlink(temp);
	}
