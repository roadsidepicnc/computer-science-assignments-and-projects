...
public class MatrixGraph
    ...
    if (!directed) {
      for (int i = 0; i != numV; ++i) {
        ...
        for (int j = 0; j != numV; ++j) {
          ...
          for(int k = 0; k < 4; k++)  edges[i][j][k] = Double.POSITIVE_INFINITY;
  ...
  public void insert(Edge edge) {
    ...
    temp[0] = edge.getWeight();
    temp[1] = edge.getDistance();
    temp[2] = edge.getTime();
    temp[3] = edge.getQuality();
  ...
  public boolean isEdge(int source, int dest) {
    return Double.POSITIVE_INFINITY != getEdgeValue(source, dest)[0];
  }
  ...
  private class Iter
    ...
    private void advanceIndex() {
      ...
      while (index != getNumV()
             && Double.POSITIVE_INFINITY == getEdgeValue(source, index)[0]);
    }
  }
